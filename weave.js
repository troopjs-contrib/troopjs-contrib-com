define([
  ...
  "require",
  "when/when"
], function (config, COMError, initialize, finalize, localRequire, when) {
  "use strict";
   ...
   * @return {Promise}
   */
  function weave(node, parent, completed) {
    var me = this;
    ...
    return when
      ...
      .otherwise(isRequireError, function (error) {
        throw new COMError("Unable to load module(s) %o in context %s because of %s. Related node was %o", error.requireModules, error.contextName, error.requireType, node);
      });
  }
